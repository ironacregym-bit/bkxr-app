...
export default function NutritionHome() {
  ...
  const [listPeople, setListPeople] = useState("1");

  async function createList() {
    const name = listName.trim() || "My List";
    const people = Math.max(1, Number(listPeople) || 1);
    setBusy(true);
    setMsg(null);
    try {
  ...
  return (
    <>
      ...
      <main className="container py-3" style={{ paddingBottom: 80, color: "#fff", borderRadius: 12, minHeight: "100vh" }}>
        ...
        {/* Tiles: left = Today snapshot; right = Shopping lists */}
          ...
          <div className="col-12 col-md-6 mb-3">
            <div className="futuristic-card p-3" style={{ height: "100%" }}>
              ...
              <div className="d-flex flex-wrap align-items-end mb-3" style={{ gap: 8 }}>
                ...
                <input className="form-control" type="number" min={1} placeholder="People" value={listPeople} onChange={(e) => setListPeople(e.target.value)} style={{ width: 110 }} />
                <button
                  className="btn btn-sm"
                  onClick={createList}
                  disabled={busy}
                  style={{ borderRadius: 24, color: "#fff", background: `linear-gradient(135deg, ${ACCENT}, #ff7f32)`, boxShadow: `0 0 14px ${ACCENT}66`, border: "none", paddingInline: 14 }}
                >
                  Create
                </button>
              </div>
