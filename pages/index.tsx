...
export default function Home() {
  ...
  // compute a stable week key (ISO YYYY-MM-DD for Monday)
  ...
  // fetch the weekly overview for the week start (single source of truth for calendar)
  const { data: weeklyOverview, isLoading: overviewLoading, mutate: mutateWeeklyOverview } = useSWR(
  ...
  useEffect(() => {
    // REQUIRE the canonical inputs: workouts data and weeklyOverview
    if (!session?.user?.email || !data?.workouts || !weeklyOverview?.days?.length) return;
    ...
    // Build base statuses for each day using workouts + completions
    for (const d of weekDays) {
      ...
      // workoutDone defaults to TRUE if no workout for that day
      ...
      statuses[dk] = {
        ...
        hasWorkout,
        workoutDone,
        nutritionLogged: false, // will be overwritten by weeklyOverview
        habitAllDone: false,    // will be overwritten by weeklyOverview
        isFriday: d.getDay() === 5,
    ...
    // Merge the weeklyOverview authoritative payload
    for (const o of weeklyOverview.days as any[]) {
      const base = statuses[o.dateKey];
      if (!base) continue;
  ...
  // when per-day items change, revalidate weeklyOverview so the calendar updates
  useEffect(() => {
    if (!session?.user?.email) return;
    // mutateWeeklyOverview is returned by useSWR (see STEP 1)
    // use a minimal debounce: only revalidate when true statuses change
    // (SWR will dedupe identical requests)
    mutate(`/api/weekly/overview?week=${weekStartKey}`).catch(() => {});
  }, [nutritionLogged, habitAllDone, workoutDoneToday, checkinComplete, session?.user?.email]);
      ...
      >
        ...
        <div className="d-flex justify-content-between text-center mb-3" style={{ gap: 8 }}>
         {weekDays.map((d, i) => {
          ...
          // neutral placeholder while weekStatus is not ready
          if (!weekStatus || !status) {
            return (
          ...
          const ringColor = status.allDone ? (isSelected ? ringGreenStrong : ringGreenMuted) : (isSelected ? accentMicro : "rgba(255,255,255,0.3)");
          const boxShadow = isSelected ? `0 0 8px ${ringColor}` : (status.allDone ? `0 0 3px ${ringColor}` : "none");
