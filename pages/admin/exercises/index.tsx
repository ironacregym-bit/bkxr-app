...
export default function AdminExercisesManager() {
  ...
  if (!session || (role !== "admin" && role !== "gym")) {
    return (
      <main className="container py-4" style={{ color: "#fff" }}>
        ...
        <Link href="/more" className="btn btn-outline-light mt-2">← Back</Link>
      </main>
    );
  }
  ...
  // Load selected exercise (read fresh to avoid stale list fields)
  const { data: selectedData, mutate: mutateSelected } = useSWR<{ exercise?: Exercise }>(
    selectedId ? `/api/exercises/get?id=${encodeURIComponent(selectedId)}` : null,
    fetcher,
  ...
  // Hydrate form when an item is selected
  useEffect(() => {
    if (!selected) return;
    setForm({
  ...
  // Basic large vs mobile split
  // We'll use Bootstrap breakpoints: md+ shows the split pane; <md shows list-first with a Create button
  return (
    <>
      ...
      <main className="container py-3" style={{ color: "#fff", paddingBottom: 90 }}>
        ...
        <div className="row gx-3">
          {/* List pane (always visible; on mobile it's the main view unless editing) */}
          ...
          <div className={`col-12 col-md-8 ${mobileEditing ? "" : "d-none d-md-block"}`}>
            <div className="futuristic-card p-3">
              ...
              <div className="row g-2">
                ...
                {/* Quick preview for Video (if YouTube/Vimeo/etc.) */}
