...
export default function CreateExercisePage() {
  ...
  async function save() {
    ...
    try {
      ...
      setForm({ exercise_name: "", type: "", equipment: "", video_url: "", met_value: "", description: "" });
    } catch (e: any) {
  ...
  return (
    <>
      ...
      <main className="container py-3" style={{ color: "#fff", paddingBottom: 90 }}>
        ...
        {msg && <div className={`alert ${msg.includes("Failed") ? "alert-danger" : "alert-success"}`}>{msg}</div>}

        <div className="bxkr-card p-3 mb-3">
          <div className="row g-2">
            <div className="col-12 col-md-6">
              <label className="form-label">Name</label>
              <input className="form-control" value={form.exercise_name}
                     onChange={(e) => setForm({ ...form, exercise_name: e.target.value })} />
            </div>
            <div className="col-6 col-md-3">
              <label className="form-label">Type</label>
              <input className="form-control" value={form.type}
                     onChange={(e) => setForm({ ...form, type: e.target.value })} placeholder="Boxing / Kettlebell / Warm up / Weights / Bodyweight" />
            </div>
            <div className="col-6 col-md-3">
              <label className="form-label">Equipment</label>
              <input className="form-control" value={form.equipment}
                     onChange={(e) => setForm({ ...form, equipment: e.target.value })} placeholder="Kettlebell / Dumbbell / Bodyweight" />
            </div>
            <div className="col-12 col-md-6">
            ...
            <div className="col-6 col-md-3">
              ...
              <input className="form-control" type="number" min={0} step="0.1" value={form.met_value}
            ...
            <ul className="small" style={{ listStyle: "none", paddingLeft: 0 }}>
              {existing.slice(0, 8).map((ex: any) => (
                <li key={ex.id} className="mb-1">
                  <code>{ex.exercise_name}</code> {ex.type ? `• ${ex.type}` : ""} {ex.equipment ? `                  <code>{ex.exercise_name}</code> {ex.type ? `• ${ex.type}` : ""} {ex.equipment ? `• ${ex.equipment}` : ""}
                </li>
              ))}
            </ul>
          )}
