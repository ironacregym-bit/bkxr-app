...
type ExerciseListItem = {
  id?: string;                // in case existing docs stored both
  exercise_id?: string;       // your new custom id
  exercise_name: string;
  type?: string;
  equipment?: string;
};
...
export default function CreateExercisePage() {
  ...
  // Load exercises (shared for sidebar + “recent”)
  const { data } = useSWR("/api/exercises?limit=500", fetcher, {
    revalidateOnFocus: false,
    dedupingInterval: 60_000
  });
  const exercises: ExerciseListItem[] = Array.isArray(data?.exercises) ? data!.exercises : [];

  // Tabs on the right panel (Form vs Paste JSON)
  ...
  // whether we mirror ID from name (for create); disabled when editing existing
  const [autoId, setAutoId] = useState<boolean>(true);

  // Selected item from the sidebar (undefined = none/new)
  const [selected, setSelected] = useState<ExerciseListItem | null>(null);

  // Hydration-safe: if a selection exists and we are in edit mode, do not mirror IDs
  useEffect(() => {
    if (selected) setAutoId(false);
  }, [selected]);

  // Keep exercise_id == exercise_name while autoId ON (create mode)
  useEffect(() => {
    if (autoId) {
  ...
  // Load a clicked exercise into the form for edit
  ...
  async function handleSave() {
    ...
    try {
      ...
      const upsert = !!selected; // editing existing
      const result = await saveOne(form, upsert);
      if (!result.ok) throw new Error(result.error);
      setMsg(upsert ? "Exercise updated ✅" : "Exercise created ✅");
      // Keep the form populated in edit; if creating, reset to new
      if (!upsert) {
        resetToNew();
      }
    } catch (e: any) {
  ...
  async function handleBulkInsert() {
    ...
    try {
      ...
      for (let i = 0; i < arr.length; i++) {
        ...
        if (result.ok) success++; else { failed++; details.push(`Item ${i + 1} (${rec.exercise_name}): ${result.error}`); }
      }
      ...
      <main className="container py-3" style={{ color: "#fff", paddingBottom: 90 }}>
        ...
        <div className="row">
          ...
          <section className="col-12 col-md-8">
                  ...
                  <div className="col-12 col-lg-6">
                    ...
                    <div className="d-flex gap-2">
                      ...
                      <div className="form-check d-flex align-items-center ms-1">
                        <input id="autoId" className="form-check-input" type="checkbox" checked={autoId} onChange={(e) => setAutoId(e.target.checked)} />
            ...
            {tab === "paste" && (
              <section className="bxkr-card p-3 mb-3" style={{ background: "rgba(255,255,255,0.05)", borderRadius: 12 }}>
                ...
                <textarea
                  ...
                  placeholder='[{"exercise_name":"Kettlebell Swings","type":"Kettlebell","equipment":"Kettlebell"}]'
                />
                <div className="mt-2 d-flex gap-2">
                  <button className="btn btn-outline-light" onClick={handleBulkInsert} disabled={bulkBusy}>
                    {bulkBusy ? "Saving…" : "Validate & Save"}
                  </button>
                  <button className="btn btn-secondary" onClick={() => setRawJson(exampleJson)} disabled={bulkBusy} title="Insert example JSON">
                    Insert example
                  </button>
                </div>
                {bulkResult && (
            ...
            {/* Mobile-only recent list (fallback when no sidebar) */}
