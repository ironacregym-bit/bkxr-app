...
function safeInterests(v: any): string[] {
  if (!Array.isArray(v)) return [];
  const out: string[] = [];
  for (const raw of v) {
    const s = safeStr(raw, 40);
    if (!s) continue;
    out.push(s);
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<WaitlistResp>) {
  ...
  const founders_interest = safeBool(body.founders_interest);
  const interests = safeInterests(body.interests);
  ...
  try {
    ...
    const prevInterests: string[] = Array.isArray(prev?.interests) ? prev.interests : [];
    const mergedInterests = Array.from(new Set([...(prevInterests || []), ...(interests || [])])).slice(0, 10);

    await docRef.set(
      {
        name: name || prev.name || "",
        phone: phone || prev.phone || null,
        goal: goal || prev.goal || null,
        consent: consent || prev.consent || false,
        founders_interest: founders_interest || prev.founders_interest || false,
        interests: mergedInterests,
        last_submitted_at: nowIso,
        source: {
          referrer: referrer || prev?.source?.referrer || null,
          utm: {
            utm_source: safeStr(utm.utm_source, 80) || prev?.source?.utm?.utm_source || null,
            utm_medium: safeStr(utm.utm_medium, 80) || prev?.source?.utm?.utm_medium || null,
            utm_campaign: safeStr(utm.utm_campaign, 120) || prev?.source?.utm?.utm_campaign || null,
            utm_content: safeStr(utm.utm_content, 120) || prev?.source?.utm?.utm_content || null,
            utm_term: safeStr(utm.utm_term, 120) || prev?.source?.utm?.utm_term || null,
          },
        },
        meta: {
          user_agent: userAgent || prev?.meta?.user_agent || null,
        },
      },
      { merge: true }
    );
