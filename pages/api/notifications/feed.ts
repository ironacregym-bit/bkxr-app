...
/**
 * Returns latest visible notifications for the signed-in user from:
 ...
 * - excludes dismissed notifications (dismissed_at != null)
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const snap = await coll.orderBy("created_at", "desc").limit(limit * 3).get();
    const now = Date.now();
    ...
    const items = snap.docs
      .map((d) => {
        ...
        const dismissed_at =
          typeof x.dismissed_at === "string"
            ? x.dismissed_at
            : x.dismissed_at?.toDate?.()
            ? x.dismissed_at.toDate().toISOString()
            : null;
      ...
      .filter((item) => {
        if (item.dismissed_at) return false;
        return true;
      })
      .filter((item) => {
