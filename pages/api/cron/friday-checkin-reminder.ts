// pages/api/cron/friday-checkin-reminder.ts
...
function getTargetFridayUTC(from: Date): Date {
...
export default async function handler(
  ...
  try {
    const now = new Date();
    const targetFriday = getTargetFridayUTC(now);
    const targetFridayYMD = ymdUTC(targetFriday);
    const runKey = `friday-checkin-${TARGET_GYM_ID}-${targetFridayYMD}`;

    const claimed = await claimRun(runKey, {
      type: "friday_checkin_reminder",
      gym_id: TARGET_GYM_ID,
      target_friday: targetFridayYMD,
    });
    ...
    if (!claimed) {
      return res.status(200).json({
        ok: true,
        skipped: true,
        reason: "Already sent for this Friday",
        runKey,
        targetGymId: TARGET_GYM_ID,
        targetFriday: targetFridayYMD,
        attempted: 0,
        succeeded: 0,
        failed: 0,
      });
    ...
    // First get active program assignments for this gym
    ...
    // Keep only assignments that are active on the target Friday
    const assignmentsActiveOnFriday = candidateAssignments.filter((a) => {
      const start = a.start_date;
      const end = a.end_date;
      if (!start || !end) return false;
      return start.getTime() <= targetFriday.getTime() && end.getTime() >= targetFriday.getTime();
    });

    if (!assignmentsActiveOnFriday.length) {
      return res.status(200).json({
        ok: true,
        runKey,
        targetGymId: TARGET_GYM_ID,
    ...
    // Cross-check user docs still belong to this gym
    const uniqueEmails = Array.from(new Set(assignmentsActiveOnFriday.map((a) => a.user_email)));
    ...
    for (const a of assignmentsActiveOnFriday) {
    ...
    for (const chunk of chunks) {
      const results = await Promise.allSettled(
        chunk.map(async (email) => {
          ...
          const title = "Friday check-in";
          const message = `Time to complete your weekly check-in for ${programName}. Tap to open Iron Acre and submit it before the weekend.`;
          ...
          return notifyInAppAndPush(
            email,
            {
              ...
              message,
              href,
              source_key: "friday_checkin_reminder",
              source_event: "friday_checkin_reminder",
              meta: {
                gym_id: TARGET_GYM_ID,
                target_friday: targetFridayYMD,
            ...
            {
              title,
              body: `Complete your weekly check-in for ${programName}.`,
              url: href,
            }
          );
        })
    ...
    console.error("[cron/friday-checkin-reminder]", err?.message || err);
    return res.status(500).json({ error: "Failed to send Friday check-in reminder" });
  }
}
