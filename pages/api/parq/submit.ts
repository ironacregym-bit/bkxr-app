  ...
  try {
    ...
    for (const key of keys) {
      ...
      if (value !== "yes" && value !== "no") {
        return res.status(400).json({
          error: "All PAR-Q questions must be answered",
        });
      }
    }
    ...
    const signedName = String(
      body.signed_name || ""
    ).trim();

    if (!signedName) {
      return res.status(400).json({
        error: "Signed name required",
      });
    }
    ...
    if (!emergencyContactName) {
      return res.status(400).json({
        error: "Emergency contact name required",
      });
    }
    ...
    if (!emergencyContactPhone) {
      return res.status(400).json({
        error: "Emergency contact phone required",
      });
    }
    ...
    if (
      body.signature_b64 &&
      isValidImageDataUrl(body.signature_b64)
    ) {
      signature_b64 =
        body.signature_b64.length <= MAX_SIG_LEN
          ? body.signature_b64
          : null;
    }
        ...
        .set(
          {
            ...
            parq_completed_at: nowIso,
            parq_response_id: parqRef.id,

            parq_requires_medical_review:
              !!body.requires_medical_review,
