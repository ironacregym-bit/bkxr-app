// File: pages/api/completions/index.ts
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // ---- MODE C: Day summary for Daily Tasks
  if (summary === "day" && dateYMD) {
    try {
      ...
      for (const [, xAny] of byId) {
        ...
        const isFreestyle = x.is_freestyle === true || String(x.is_freestyle).toLowerCase() === "true";
        if (isFreestyle && ts > freestyleLatestTs) {
          freestyleLatestTs = ts;

          const act = typeof x.activity_type === "string" ? (x.activity_type as string) : null;
          ...
          const cals = Number.isFinite(Number(x.calories_burned)) ? Number(x.calories_burned) : null;
          const wt = Number.isFinite(Number(x.weight_completed_with)) ? Number(x.weight_completed_with) : null;
  ...
  try {
    ...
    const results = docs.map((c) => {
      const completedIso = isoFromAny(c.completed_date) || isoFromAny(c.started_at) || isoFromAny(c.created_at);
      ...
      return {
        ...
        started_at: isoFromAny(c.started_at),
        created_at: isoFromAny(c.created_at),
        updated_at: isoFromAny(c.updated_at),
